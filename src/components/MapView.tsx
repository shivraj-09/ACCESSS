...
export default function MapView({
  ...
  const visibleReports = reports.filter(
    (report) => report.id !== focusReportId,
  )
  ...
  return (
    <div className="relative h-full min-h-[350px] w-full overflow-hidden">
            ...
            <Popup>
              <div className="min-w-[190px]">
                ...
                <Link
                  ...
                  className="mt-3 inline-block text-sm font-bold text-slate-950"
                >
                  View report →
                </Link>
              </div>
      ...
      {!focusLocation && (
        <div className="absolute left-4 top-4 z-[1000] rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-lg">
          ...
          <p className="mt-1 text-xs text-slate-500">
            {loading
              ? 'Loading reports...'
              : `${visibleReports.length} confirmed report${
      ...
      {!focusLocation && (
        <div className="absolute bottom-6 right-4 z-[1000] flex flex-col gap-2">
          <button
            ...
            onClick={locateUser}
            disabled={locating}
            className="flex h-12 w-12 items-center justify-center rounded-2xl border border-slate-200 bg-white shadow-lg transition hover:bg-slate-50 disabled:opacity-60"
          ...
          <button
            ...
            className="flex h-12 w-12 items-center justify-center rounded-2xl border border-slate-200 bg-white shadow-lg transition hover:bg-slate-50"
      ...
      {!focusLocation && (
        <div className="absolute bottom-6 left-4 z-[1000] hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-lg sm:block">
          ...
          <div className="mt-3 space-y-2 text-xs text-slate-600">
            <div className="flex items-center gap-2">
              <span className="flex h-6 w-6 items-center justify-center rounded-full bg-slate-950 font-bold text-white">
                !
              </span>
              Barrier
            </div>
      ...
      {focusLocation && loading && (
        <div className="absolute left-4 top-4 z-[1000] rounded-xl bg-white px-4 py-2 text-xs font-semibold shadow-lg">
